    ...
    )
{
    ...
    if (pPhyMod != NULL)
    {
        ...
        for (iBone = 0; iBone < psmSkinMap->m_cbiBones; iBone++)
        {
            ...
            if(!psmSkinMap->m_rgbiBones[iBone].m_pBoneNode)
            {
                MessageBox(0,"Invalid bone found. Trying to skip.\n This'll probably crash or produce garbage. The invalid bone should be fixed.",0,0);
                continue;
            }
    ...
    ) 
{
    ...
    DlgOptions.m_xFormat = DXFILEFORMAT_TEXT;
    DlgOptions.m_bSavePatchData = FALSE;
    DlgOptions.m_bSaveAnimationData = TRUE;
    DlgOptions.m_bLoopingAnimationData = FALSE;
