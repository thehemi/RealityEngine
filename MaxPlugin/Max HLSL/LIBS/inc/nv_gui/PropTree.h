...
#include "PropTreeItemFileBrowse.h"
#include "PropTreeItemGrid.h"
...
#define PTN_FILEEDIT				(PTN_FIRST-13)
...
public:
    ...
    static const COLORREF & GetReadOnlyColor();
    static const COLORREF & GetColor();
	...
	void EnsureVisible(CPropTreeItem* pItem, BOOL bExpand = TRUE);
    ...
    // set the image list
    void SetImageList(CImageList & images,CImageList & ImagesDisabled);

    // return the image list
    CImageList & GetImageList();
    CImageList & GetImageListDisabled();

    unsigned int SetStateImageIdx(CPropTreeItem::ItemState state, unsigned int idx);

    unsigned int GetStateImageIdx(CPropTreeItem::ItemState state);
    
    BOOL        ShowReadOnlyAttributes();


protected:
    // Image List
    CImageList      m_Images, m_ImagesDisabled;
    ...
    static COLORREF m_ReadOnlyTextColor;
    ...
    static std::map<CPropTreeItem::ItemState,unsigned int> m_ItemStateIdx;
public:
	//
	// functions used by CPropTreeItem (you normally dont need to call these directly)
	//

    BOOL IsFilter(DWORD dwTest);
    void Filter(DWORD dwAdd, DWORD dwRemove);
    ...
    void    ExpandAll(CPropTreeItem* pItem, BOOL bExpand = TRUE);
...
protected:
    ...
    void RecEnsureVisible(CPropTreeItem* pItem, BOOL bExpand);
// Overrides
...
private:
    ...
    static BOOL CALLBACK EnumGetSelection(CPropTree*, CPropTreeItem* pItem, LPARAM);
