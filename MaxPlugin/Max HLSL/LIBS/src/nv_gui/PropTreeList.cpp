...
void CPropTreeList::OnPaint() 
{
    ...
    hOldBrush = memdc.SelectObject(GetSysColorBrush(COLOR_BTNFACE));
    ...
    hOldBrush = memdc.SelectObject(CBrush::FromHandle(GetSysColorBrush(COLOR_BTNFACE)));
    memdc.PatBlt(rc.left, rc.top, m_pProp->GetColumn(), rc.Height(), PATCOPY);
    memdc.SelectObject(hOldBrush);
	...
	{
        if (m_ShowReadOnlyAttributes || pItem->IsReadOnly() == FALSE || pItem->GetChild())
        {
...
BOOL CPropTreeList::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
	if (nHitTest==HTCLIENT)
	{
		...
		{
			case HTCOLUMN:
				SetCursor(LoadCursor(AfxGetResourceHandle(), MAKEINTRESOURCE(IDC_SPLITTER)));
				return TRUE;
...
void CPropTreeList::OnRButtonDown(UINT, CPoint point) 
{
    POINT pt;
    pt.x = point.x;
    pt.y = point.y;
    ...
    CMenu OptionMenu;
    //OptionMenu.SetFont(m_pProp->GetNormalFont())
    OptionMenu.LoadMenu(IDR_CONNECTION_EDITOR_OPTIONS);
    CString s;
    OptionMenu.GetMenuString(ID_READONLYATTRIBUTES, s, MF_BYCOMMAND);
    CString s_mod = m_ShowReadOnlyAttributes ? "Hide " : "Show ";
    s_mod += s;
    OptionMenu.ModifyMenu(ID_READONLYATTRIBUTES, MF_BYCOMMAND, ID_READONLYATTRIBUTES, s_mod);
    OptionMenu.TrackPopupMenu(TPM_LEFTALIGN |TPM_RIGHTBUTTON,pt.x,pt.y,AfxGetMainWnd());
}

void CPropTreeList::OnReadOnlyAttributes()
{
}
