...
void Xrefutil::Init()
{
	m_pInterface = GetCOREInterface();
}

void Xrefutil::Destroy()
{
}


// Function name	: Xrefutil::AddNewXrefObject
// Description	    : TIM JOHNSON CREATES FOR PREFAB MANAGER
// Return type		: Head Node 
// Argument         : HWND hWnd
INode* Xrefutil::AddNewXrefObject(string filename)
{
	// *** AddNewXrefObject ***
	//
	...
	// 2) Merges in this file
	// 3) ConvertSelectedToXrefObject for all nodes
	// 4) Gives them unique ids
	// 5) Tags them to a group if needed
	...
	if (!pRootNode) {
		MessageBox(0,"Couldn't find root node in current file. This is fatal",0,0);
		return NULL;
	}
	...
	if (! m_pInterface->MergeFromFile(filename.c_str(), TRUE, FALSE, FALSE, MERGE_DUPS_MERGE/*MERGE_DUPS_PROMPT*/, NULL)) {
		// error, merge failed
	...
	// Look for a group
	bool needsGroup = m_objnamesholder.Count() > 1;

	NameMaker *nm = m_pInterface->NewNameMaker();
	INodeTab group;
	INode* headNode = NULL;
	for (int delme = 0; delme < m_objnamesholder.Count(); delme++) { 
		INode* pNode = m_pInterface->GetINodeByName(m_objnamesholder[delme]->data());
		// Make first node the group head, or save the group head if the file already has one
		if(needsGroup)
			group.Append(1,&pNode);
		else
			headNode = pNode;

		// Make X-Ref!
		NodeToXref(pNode, CStr(filename.c_str()), false, true);

		// Make name unique
		CStr nodename = (string("P_") + pNode->GetName()).c_str();
		nm->MakeUniqueName(nodename);
		pNode->SetName(nodename.data());
	}

	// Unselect anything else currently selected
	m_pInterface->ClearNodeSelection(FALSE);

	// No group, so select sole object
	if(!needsGroup){
		INode* node = m_pInterface->GetINodeByName(m_objnamesholder[0]->data());
		theHold.Begin();
		m_pInterface->SelectNode(node);
		TSTR undostr; undostr.printf("Select");
		theHold.Accept(undostr);
	}

	// Make a group with unique name & selects it
	if(needsGroup){
		string name = filename.data();
		name = name.substr(name.find_last_of("\\")+1);
		name += " ";
		CStr cName = name.c_str();
		nm->MakeUniqueName(cName);
		headNode = m_pInterface->GroupNodes(&group,&cName,TRUE);
		m_pInterface->CloseGroup(&group);
	}
...
bool Xrefutil::DoOpenSaveDialog(TSTR &fileName, bool bOpen)
{
    ...
    ofn.nMaxFileTitle    = 0;
    ofn.lpstrInitialDir  = fileName;//m_pInterface->GetDir(APP_SCENE_DIR);
...
void Xrefutil::AddNewXrefScene(string filename)
{
    // *** AddNewXrefScene ***
    //
    ...
    //    return;
    //}    
    pRootNode->AddNewXRefFile(CStr(filename.c_str()), TRUE);
    m_pInterface->RedrawViews(m_pInterface->GetTime());
}
...
// Return type		: Head node of new xref 
// Argument         : HWND hWnd
INode* Xrefutil::ConvertSelectedToXrefObject(string& outFile, HWND hWnd)
{
   ...
   /* if (m_pInterface->GetSelNodeCount() != 1) {
	...
	for(int i=0;i<m_pInterface->GetSelNodeCount();i++){
		INode* node = m_pInterface->GetSelNode(i);
		NodeToXref(node, filename, false);
		group.Append(1,&node);
	}

	INode* head = m_pInterface->GetSelNode(0);
	
	// Make a group with unique name & selects it
	// This prevents users altering an xref grouping
	if(m_pInterface->GetSelNodeCount() > 1){
		string name = filename.data();
		name = name.substr(name.find_last_of("\\")+1);
		name += " ";
		...
		NameMaker *nm = m_pInterface->NewNameMaker();
		nm->MakeUniqueName(cName);
		head = m_pInterface->GroupNodes(&group,&cName,TRUE);
		m_pInterface->CloseGroup(&group);
	}
...
void Xrefutil::ExportXrefObjects(HWND hWnd)
{
    ...
    int dlgres = DialogBoxParam(g_hInstance, 
