...
public:
	...
	// Returns head node of new object
	INode* AddNewXrefObject(string filename);
	INode* ConvertSelectedToXrefObject(string& outFile, HWND hWnd);
...
// Just a pointer to the interface class IObjXRefManager.
    ...
    // (xref scene methods)
	void AddNewXrefScene(string filename);
