...
/// Handle that ties Rigid Body to either an unmanaged Actor or to the managed API.
struct ENGINE_API RigidBodyIdentifier
{
	/// If managed, managedIndex will be < 0 and appropriate callbacks to the managed system will be triggered within the general rigid body physics system.
    int managedIndex;
	/// rigid body to which this identifier is tied
    void * rigidBody;
	/// Allows association with a Parent Actor ifin the unmanaged system
	Actor* Parent;
	...
	/// creates a new identifier for the specified rigid body
    static RigidBodyIdentifier CreateFromRigidBody(void * rigidBody)
    {
        RigidBodyIdentifier id;
        id.managedIndex=0;
        id.rigidBody=rigidBody;
		id.Parent = NULL;
		id.IgnoreTerrain = false;
        return id;
    }
};
...
public:	
    ...
    {
        RigidBodyIdentifier * id =(RigidBodyIdentifier*)controller->GetRigidBody()->GetUserData();
        if (id->managedIndex == 0)
	        ((TokamakBody*)id->rigidBody)->RigidController(controller);
    };	
};
...
public:
    ...
    static void Tick(class World* world, float TimeScale, int MaxSteps);
	...
	void CreateRayTestBody();
	void QueueForDeletion(neRigidBody* rBody);
private:
	vector<neRigidBody*> RBDeletionQueue;
};
