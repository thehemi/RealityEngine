...
{
    RenderDevice::Instance()->ShowCursor(false,false);
    g_Status = status;
    g_Percent = percent;
...
{
	return (m_GUISystem->DesktopVisible() || m_Console.IsOn() || Editor::Instance()->GetEditorMode());
}
...
void Game::ToggleMainMenu()
{
	...
	if(!IsInGame())
		m_GUISystem->ShowDesktop(true);
	else 
		m_GUISystem->ShowDesktop(!m_GUISystem->DesktopVisible());
