...
protected:
	...
	typedef vector<Data> HashBucketList_t;
	vector<HashBucketList_t>	m_Buckets;
	...
	m_KeyFunc( keyFunc )
{
	...
	{
		m_Buckets[ndxBucket].resize( initCount );
		//m_Buckets[ndxBucket].( growCount ); /// SetGrowSize = grow?
	}
...
inline int CUtlHash<Data>::Count( void )
{
	...
	int bucketCount = m_Buckets.size();
	for( int ndxBucket = 0; ndxBucket < bucketCount; ndxBucket++ )
	{
		count += m_Buckets[ndxBucket].size();
	}
...
inline UtlHashHandle_t CUtlHash<Data>::Insert( Data const &src )
{
	...
	int	ndx = m_Buckets[ndxBucket].size();
	m_Buckets[ndxBucket].push_back( src );
...
inline UtlHashHandle_t CUtlHash<Data>::AllocEntryFromKey( Data const &src )
{
	...
	/// add the key/data pair to the "bucket"
	int	ndx = m_Buckets[ndxBucket].size();
	m_Buckets[ndxBucket].resize(m_Buckets[ndxBucket].size()+1);
...
inline UtlHashHandle_t CUtlHash<Data>::Find( Data const &src )
{
	...
	int ndxKeyData;
	int keyDataCount = m_Buckets[ndxBucket].size();
	for( ndxKeyData = 0; ndxKeyData < keyDataCount; ndxKeyData++ )
	{
		if( m_CompareFunc( m_Buckets[ndxBucket][ndxKeyData], src ) )
			break;
	}
