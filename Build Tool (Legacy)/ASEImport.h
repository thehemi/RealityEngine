...
private:
	...
	Vector GetVector(const string& line, int offset);
	string GetWord(const string& s, int index);
	float GetFloat(const string& line, int index);
	int   GetInt(const string& line, int index);
	void DummyRead(const string& line);
	

	// Top-level
	void ReadSCENE();
	vector<Material>	ReadMATERIAL_LIST(string line);
	GeomObject			ReadGEOMOBJECT();
	LightKeys			ReadLIGHT();
	void				ReadHELPER();
	...
	// Sub-level
	void ReadTMAnimation(vector<Matrix>& keyframes, vector<float>& frameTimes);
	Matrix ReadTM();
	Matrix ReadMatrix(string& startLine = string(""), int offset = 0);
...
public:
	SceneData scene;
	ASEImport(string filename, bool& success);
};
...
// Node related ID's
#define ID_NODE_PROPBUFFER		_T("*NODE_PROPBUFFER")
...
#define ID_TM_TRACK				_T("*CONTROL_TM_TRACK") // Tim Johnson added
...
#define ID_SPECULAR_LEVEL		_T("*MATERIAL_SPECULAR_LEVEL") 
