...
//--------------------------------------------------------------------------------------
// TIM: BLOCKER MESH
HRESULT CPRTSimulator::Run( IDirect3DDevice9* pd3dDevice, SIMULATOR_OPTIONS* pOptions, CPRTMesh* pPRTMesh, ID3DXMesh* blockerMesh )
{
    if( IsRunning() ) 
        return E_FAIL;
...
DWORD CPRTSimulator::PRTSimulationThreadProc()
{
    ...
    V( D3DXCreatePRTEngine( pMesh, bExtractUVs, m_pBlockerMesh, &m_pPRTEngine ) );
	...
	if(m_Options.bSubsurfaceScattering)
		pDataTotal->ScaleBuffer(3);
/*
	LPD3DXTEXTUREGUTTERHELPER  helper;
	D3DXCreateTextureGutterHelper (pDataTotal->GetHeight(),pDataTotal->GetWidth(),m_pPRTMesh->m_pMesh,2.0,&helper);
...
*/
    ...
    m_pPRTMesh->CompressBuffer( D3DXSHCQUAL_SLOWHIGHQUALITY, 1, 24 );
