...
#	define swapLE(n)
#	define getLE(n) (n)
	FORCEINLINE void swapBE(u32 &n) { n = BOSWAP32(n); }
	FORCEINLINE void swapBE(u16 &n) { n = BOSWAP16(n); }
	FORCEINLINE u32 getBE(u32 n) { return BOSWAP32(n); }
	FORCEINLINE u16 getBE(u16 n) { return BOSWAP16(n); }
	FORCEINLINE void swapBE(s32 &n) { n = BOSWAP32((u32)n); }
	FORCEINLINE void swapBE(s16 &n) { n = BOSWAP16((u16)n); }
	FORCEINLINE s32 getBE(s32 n) { return BOSWAP32((u32)n); }
	FORCEINLINE s16 getBE(s16 n) { return BOSWAP16((u16)n); }
#else
#	define swapBE(n)
#	define getBE(n) (n)
	FORCEINLINE void swapLE(u32 &n) { n = BOSWAP32(n); }
	FORCEINLINE void swapLE(u16 &n) { n = BOSWAP16(n); }
	FORCEINLINE u32 getLE(u32 n) { return BOSWAP32(n); }
	FORCEINLINE u16 getLE(u16 n) { return BOSWAP16(n); }
	FORCEINLINE void swapLE(s32 &n) { n = BOSWAP32((u32)n); }
	FORCEINLINE void swapLE(s16 &n) { n = BOSWAP16((u16)n); }
	FORCEINLINE s32 getLE(s32 n) { return BOSWAP32((u32)n); }
	FORCEINLINE s16 getLE(s16 n) { return BOSWAP16((u16)n); }
#endif
...
// Masking
#define CLR_BITS(n, mask) n ^= n & (mask)
#define SET_BITS(n, mask) n |= (mask)
#define GET_BITS(n, mask) ( n & (mask) )
#define MAKE_MASK(bits) ( (1 << (bits)) - 1 )	/* gives a warning for bits=31, doesn't work for bits>=32 */
...
FORCEINLINE cat::u32 next_highest_power_of_2(cat::u32 n)
{
