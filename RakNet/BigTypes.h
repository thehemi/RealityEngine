...
#define BIGHALFSIZE(T, var_name) big::word (var_name)[BIGWORDCOUNT(T) / 2] /* WARNING: invalid w/ u128 */
	...
	// assignment
	BIGONETYPE FORCEINLINE void zero(T &n); // n = 0
	BIGONETYPE FORCEINLINE void usetw(T &a, word b); // a = b, zero-extend
	BIGONETYPE FORCEINLINE void ssetw(T &a, word b); // a = b, sign-extend
	BIGONETYPE FORCEINLINE void set(T &a, T &b); // a = b
	BIGTWOTYPES FORCEINLINE void usetlow(Bigger &a, T &b); // a_low = b (zero-extend)
	BIGTWOTYPES FORCEINLINE void ssetlow(Bigger &a, T &b); // a_low = b (sign-extend)
	BIGTWOTYPES FORCEINLINE void sethigh(Bigger &a, T &b); // a_high = b
	BIGTWOTYPES FORCEINLINE void takelow(T &a, Bigger &b); // a = b_low
	BIGTWOTYPES FORCEINLINE void takehigh(T &a, Bigger &b); // a = b_high
	...
	BIGONETYPE FORCEINLINE bool equal(T &a, T &b); // a == b
	BIGONETYPE FORCEINLINE bool isZero(T &n); // a == 0
...
#define BIGINTFAST FORCEINLINE Int<T> & /* operation is done to self, returns itself */
#define BIGINTSLOW Int<T> /* new object is created and returned */
	...
	// n = 0
	BIGONETYPE FORCEINLINE void zero(T &n)
	{
		memset(n, 0, sizeof(T));
	}

	// a = b, zero-extend
	BIGONETYPE FORCEINLINE void usetw(T &a, word b)
	{
		a[0] = b;
	...
	// a = b, sign-extend
	BIGONETYPE FORCEINLINE void ssetw(T &a, word b)
	{
		a[0] = b;
	...
	// a = b
	BIGONETYPE FORCEINLINE void set(T &a, T &b)
	{
		memcpy(a, b, sizeof(T));
	}

	// a_low = b (zero-extend)
	BIGTWOTYPES FORCEINLINE void usetlow(Bigger &a, T &b)
	{
		memcpy(a, b, sizeof(T));
	...
	// a_low = b (sign-extend)
	BIGTWOTYPES FORCEINLINE void ssetlow(Bigger &a, T &b)
	{
		memcpy(a, b, sizeof(T));
	...
	// a_high = b
	BIGTWOTYPES FORCEINLINE void sethigh(Bigger &a, T &b)
	{
	...
	// a = b_low
	BIGTWOTYPES FORCEINLINE void takelow(T &a, Bigger &b)
	{
		memcpy(a, b, sizeof(T));
	}

	// a = b_high
	BIGTWOTYPES FORCEINLINE void takehigh(T &a, Bigger &b)
	{
	...
	// a == b
	BIGONETYPE FORCEINLINE bool equal(T &a, T &b)
	{
		return memcmp(a, b, sizeof(T)) == 0;
	}

	// a == 0
	BIGONETYPE FORCEINLINE bool isZero(T &n)
	{
	...
	BIGONETYPE void sshiftRight(T &n, u32 s)
	{
		...
		word filler = BIGHIGHBIT(n) ? BIGWORD_ALLBITS_MASK : 0;
...
done_already:
	...
	BIGONETYPE FORCEINLINE Int<T>::Int()
	{
		big::zero(raw);
	}

	BIGONETYPE FORCEINLINE Int<T>::Int(word n)
	{
		ssetw(raw, n);
	}
	...
	BIGONETYPE FORCEINLINE Int<T>::Int(std::string &s)
	{
		fromString(s, raw);
	}
	...
	BIGONETYPE FORCEINLINE Int<T>::Int(T &n)
	{
		set(raw, n);
	}

	BIGONETYPE FORCEINLINE Int<T>::operator T &()
	{
		return raw;
	}
	...
	BIGONETYPE FORCEINLINE /* fast */ bool Int<T>::operator>(T &n)
	{
	...
	BIGONETYPE FORCEINLINE /* fast */ bool Int<T>::operator!()
	{
		return isZero(raw);
	}
	...
	BIGONETYPE FORCEINLINE /* fast */ std::string Int<T>::str()
	{
		return toString(raw);
	}
