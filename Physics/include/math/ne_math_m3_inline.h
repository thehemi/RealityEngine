...
NEINLINE neM3 & neM3::SetTranspose( neM3 & m )
{
	if (this == &m)
	{
		neSwap(M[1].n.X, M[0].n.Y);  
		neSwap(M[2].n.X, M[0].n.Z); 

		//neSwap(M[0].Y, M[1].X);  
		neSwap(M[2].n.Y, M[1].n.Z); 
...
NEINLINE neV3 neM3::TransposeMulV3(const neV3 & V)
{
	...
	tmp[0] = M[0].v[0] * V.n.X + M[0].v[1] * V.n.Y + M[0].v[2] * V.n.Z;
	tmp[1] = M[1].v[0] * V.n.X + M[1].v[1] * V.n.Y + M[1].v[2] * V.n.Z;
	tmp[2] = M[2].v[0] * V.n.X + M[2].v[1] * V.n.Y + M[2].v[2] * V.n.Z;
...
NEINLINE neV3 operator * ( const neM3& m1, const neV3& v)
{
	...
	tmp[0] = m1.M[0].v[0] * v.n.X + m1.M[1].v[0] * v.n.Y + m1.M[2].v[0] * v.n.Z;
	tmp[1] = m1.M[0].v[1] * v.n.X + m1.M[1].v[1] * v.n.Y + m1.M[2].v[1] * v.n.Z;
	tmp[2] = m1.M[0].v[2] * v.n.X + m1.M[1].v[2] * v.n.Y + m1.M[2].v[2] * v.n.Z;
...
{
    M[0][0] = V1.n.X;  M[0][1] = V1.n.Y;  M[0][2] = V1.n.Z;
    M[1][0] = V2.n.X;  M[1][1] = V2.n.Y;  M[1][2] = V2.n.Z;
    M[2][0] = V3.n.X;  M[2][1] = V3.n.Y;  M[2][2] = V3.n.Z;
}
...
NEINLINE neM3 neM3::operator ^ (const neV3 & v) const
{
	...
	ret[0][0] = M[1][0] * v.n.Z - M[2][0] * v.n.Y;
	ret[0][1] = M[1][1] * v.n.Z - M[2][1] * v.n.Y;
	ret[0][2] = M[1][2] * v.n.Z - M[2][2] * v.n.Y;

	ret[1][0] = M[0][0] * -v.n.Z + M[2][0] * v.n.X;
	ret[1][1] = M[0][1] * -v.n.Z + M[2][1] * v.n.X;
	ret[1][2] = M[0][2] * -v.n.Z + M[2][2] * v.n.X; 
	
	ret[2][0] = M[0][0] * v.n.Y - M[1][0] * v.n.X;
	ret[2][1] = M[0][1] * v.n.Y - M[1][1] * v.n.X;
	ret[2][2] = M[0][2] * v.n.Y - M[1][2] * v.n.X;
...
NEINLINE neM3 operator ^ (const neV3 & v, const neM3 & matrix)
{
	...
	ret[0][0] = -v.n.Z * matrix[0][1] + v.n.Y * matrix[0][2];
	ret[0][1] =  v.n.Z * matrix[0][0] - v.n.X * matrix[0][2];
	ret[0][2] = -v.n.Y * matrix[0][0] + v.n.X * matrix[0][1];

	ret[1][0] = -v.n.Z * matrix[1][1] + v.n.Y * matrix[1][2]; 
	ret[1][1] =  v.n.Z * matrix[1][0] - v.n.X * matrix[1][2]; 
	ret[1][2] = -v.n.Y * matrix[1][0] + v.n.X * matrix[1][1]; 
	
	ret[2][0] = -v.n.Z * matrix[2][1] + v.n.Y * matrix[2][2]; 
	ret[2][1] =  v.n.Z * matrix[2][0] - v.n.X * matrix[2][2]; 
	ret[2][2] = -v.n.Y * matrix[2][0] + v.n.X * matrix[2][1]; 
