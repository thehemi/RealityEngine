...
/// Purpose: Generic spatial partitioning tree
//
/// 
//
//
/// Note: Enumerators are great ways to do things to entities
//
//=============================================================================
...
struct Ray_t
{
	...
	void Init( Vector const& start, Vector const& end )
	{
		...
		m_IsSwept = (m_Delta.DotSelf() != 0);

		m_Extents.Set(0,0,0);
		m_IsRay = true;

		/// Offset m_Start to be in the center of the box...
		m_StartOffset.Set(0,0,0);
		m_Start = start;
	}
	...
	void Init( Vector const& start, Vector const& end, Vector const& mins, Vector const& maxs )
	{
		...
		m_Extents = maxs - mins;
		m_Extents *= 0.5f;
		m_IsRay = (m_Extents.DotSelf() < 1e-6);
...
enum
{
	PARTITION_ENGINE_PREFABS		= (1 << 0),		/// every edict_t that isn't SOLID_TRIGGER or SOLID_NOT (and static props)
	PARTITION_ENGINE_ACTORS		= (1 << 1),		/// every edict_t that IS SOLID_TRIGGER
	...
	//PARTITION_ENGINE_ACTORS	= (1 << 4),		/// everything in solid & trigger except the static props, includes SOLID_NOTs
