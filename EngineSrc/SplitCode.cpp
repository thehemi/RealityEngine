...
// Usually don't care about split warnings, because it's done in realtime for shadows and things
#define DebugNotice(x)
...
bool FaceCrossesPlane(Plane& plane, Face& face, bool includeBorderFaces = false)
{
	...
	for (int i = 0; i < 3; i++)
	{
		float dist = plane.norm.Dot(face.v[i].position) + plane.d;
...
int ClipPolygon(Vertex* poly, Plane& plane, int FLAG)
{
	...
	for(int i=0;i<3;i++)
    {
		...
		float d1 = plane.norm.Dot(v1.position) + plane.d;
		float d2 = plane.norm.Dot(v2.position) + plane.d;
		...
		Vertex newVert;
		float s = d1/(d1-d2);
		newVert.position = v1.position + s*(v2.position - v1.position);
		newVert.tex = v1.tex + s*(v2.tex - v1.tex);
		newVert.normal = (v1.normal + s*(v2.normal - v1.normal)).Normalized(); // NOTE: Can you do this interpolation for normals?
...
vector<Face> SplitTriangle(Plane& plane, Face& triangle, BBox& box, int FLAG)
{
	...
	if(verts != 3)
    {
		if(verts != 4)
        {
			DebugNotice("Error: Clipped triangle gave a polygon with %d points",verts);
		}
...
void SplitByBox(BBox& box, vector<Face>& faces, vector<Face>& boxFaces)
{
	...
	for(int i=0;i<faces.size();i++)
    {
		...
		if(j == 6){
			...
			int b1 = Inside(box,faces[i].v[0].position);
			int b2 = Inside(box,faces[i].v[1].position);
			int b3 = Inside(box,faces[i].v[2].position);

			if((b1 == 1 || b2 == 1 || b3 == 1) && (b1 == 0 || b2 == 0 || b3 == 0)){
				DebugNotice("ERRROR - SHOULD HAVE BEEN SPLIT PROPERLY - PART IS STILL IN AND OUT THOUGH");
			...
			if(!(b1 == -1 && b2==-1 && b3 == -1))
				DebugNotice("IMPOSSIBLE - NOT INSIDE, OUTSIDE, OR FULLY BORDERING!!!");
			...
			if(triBoxOverlap(box,faces[i])){
				...
				DebugNotice("ERROROR all points bordering and tri is outside - tri must be degenerate!");
...
int triBoxOverlap(BBox& box, Face& face)
{
	...
	float* triverts[3];
	triverts[0] = (float*)&face.v[0].position;
	triverts[1] = (float*)&face.v[1].position;
	triverts[2] = (float*)&face.v[2].position;
