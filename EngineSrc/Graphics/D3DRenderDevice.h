...
public:
    ...
    bool    FindBestFullscreenMode( bool bRequireHAL, bool bRequireREF, int width, int height, int depth  );
    bool    ChooseInitialD3DSettings();
    HRESULT Initialize3DEnvironment(bool bReset = false);
...
public:
	...
	LPDIRECT3DSWAPCHAIN9 chain;			   /// The swap chain, used for Present()
    ...
    virtual HRESULT InitDeviceObjects()                        { return EventCallback(RE_INITIALIZE); }
    virtual HRESULT RestoreDeviceObjects()                     { return EventCallback(RE_RESTORE); }
    virtual HRESULT FrameMove()                                { return S_OK; }
    virtual HRESULT Render()                                   { return S_OK; }
    virtual HRESULT InvalidateDeviceObjects()                  { return EventCallback(RE_INVALIDATE); }
    virtual HRESULT DeleteDeviceObjects()                      { return EventCallback(RE_DELETE); }
    virtual HRESULT FinalCleanup()                             { return EventCallback(RE_FINALCLEANUP);
	; }
...
public: /// Tim's additions
	...
	typedef HRESULT (*function)(RenderEvent event);
	function EventCallback;

	BOOL	IsFullscreen(){ return !m_bWindowed; }
	LPDIRECT3DDEVICE9 GetDevPtr(){ return m_pd3dDevice; }
	void	ShowCursor(bool bShow);
	VOID    Destroy();
	HRESULT SetVideoMode(int SizeX, int SizeY, int ColorDepth, bool FullScreen, bool VSync);

    HRESULT Create( HINSTANCE hInstance, HWND hWnd, int SizeX, int SizeY, int ColorDepth, bool						FullScreen, bool VSync, function NewEventCallback );
    LRESULT MsgProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam );
	HRESULT BeginRender();
	HRESULT EndRender();
	
	/// PRIVATE!!!
	bool PrepareSettings(int SizeX, int SizeY, int ColorDepth, bool FullScreen, bool VSync);

	BOOL			  WantHarwareTNL;
	BOOL			  m_bUseVSync; /// Whether to Lock to vertical sync when in fullscreen
	BOOL			  m_bCursorVisible; /// App toggles
	BOOL			  m_bCenterCursor;/// App toggles
};
