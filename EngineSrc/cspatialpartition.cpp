...
// there was necessary for a constant-time element removal.
//
//
//=============================================================================
#include <stdafx.h>
...
#include "..\shared\utlhash.h"
#include "..\shared\utllinkedlist.h"
#include "FXManager.h"

//-----------------------------------------------------------------------------
// What's the max size of the smallest spatial partition?
//-----------------------------------------------------------------------------
#define SPATIAL_SIZE		(64) // Meters
...
//-----------------------------------------------------------------------------
/// The spatial partition class, gathers static elements within a volume of the World
//-----------------------------------------------------------------------------
class CSpatialPartition : public ISpatialQuery, ISpatialPartition
{
public:
...
//-----------------------------------------------------------------------------
CSpatialPartition	g_SpatialPartition;

ISpatialPartition* SpatialPartition(){
	return (ISpatialPartition*)&g_SpatialPartition;
}
...
/*
	...
	*/
	m_Node.push_back(AreaNode_t());
	int node = m_Node.size() - 1;
...
void CSpatialPartition::Init( const Vector& worldmin, const Vector& worldmax )
{
	...
	// TIM: Changed Z to Y
	int intMin[3];
	intMin[0] = (int)floor(worldmin[0] / SPATIAL_SIZE);
	intMin[1] = (int)floor(worldmin[1] / SPATIAL_HEIGHT) * NODE_HEIGHT_RATIO;
	intMin[2] = (int)floor(worldmin[2] / SPATIAL_SIZE);

	int intMax[3];
	for (i = 0; i < 3; ++i )
	{
		int intSize = (i != 1) ? 1 : NODE_HEIGHT_RATIO;
	...
	// Y must be NODE_HEIGHT_RATIO * a power of 2
	assert( IsPowerOfTwo( intMax[0] - intMin[0] ) );
	assert( IsPowerOfTwo((intMax[1] - intMin[1] ) / NODE_HEIGHT_RATIO ) );
	assert( IsPowerOfTwo( intMax[2] - intMin[2] ) );
...
SpatialPartitionHandle_t CSpatialPartition::CreateHandle( IHandleEntity *pHandleEntity )
{
	...
	m_Handle[handle].m_Min.Set( FLT_MAX, FLT_MAX, FLT_MAX );
	m_Handle[handle].m_Max.Set( FLT_MIN, FLT_MIN, FLT_MIN );
	...
	ISpatialLeafEnumerator* pEnum, int context )
{
	...
	while (node >= 0)
	{
		...
		else
		{
			...
			Vector split = ray.m_Start+ray.m_Delta*splitfrac;
...
//		if ((tStart <= -extents) && (tEnd <= -extents) )
		...
		Vector split[2];
		split[0]=ray.m_Start+ray.m_Delta*splitfracBehind;
		split[1]=ray.m_Start+ray.m_Delta*splitfracInFront;
...
static inline bool IsSameMultipleOfFactor2( const Vector& src1, const Vector& src2 )
{
	if ( floor(src1[0] * INV_SPATIAL_SIZE ) != floor(src2[0] * INV_SPATIAL_SIZE) )
		return false;
	if ( floor(src1[1] * INV_SPATIAL_HEIGHT ) != floor(src2[1] * INV_SPATIAL_HEIGHT ) )
		return false;
	if ( floor(src1[2] * INV_SPATIAL_SIZE ) != floor(src2[2] * INV_SPATIAL_SIZE) )
		return false;
...
#if 1
	// TIM: Changed Z to Y
	Vector bucketSize( SPATIAL_SIZE, SPATIAL_HEIGHT, SPATIAL_SIZE );
...
//-----------------------------------------------------------------------------
/// Base class for performing BSP element enumeration
//-----------------------------------------------------------------------------
...
public:
	...
	bool _fastcall EnumerateElement( int partitionHandle, int enumId )
	{
		// Get at the handle
...
public:
	...
	{
		BBox box(handleInfo.m_Min, handleInfo.m_Max );
		return box.IsPointInBox( m_Point );
	}
...
public:
	...
	{
		BBox box(handleInfo.m_Min, handleInfo.m_Max );
		BBox other(m_Min,m_Max);

		return box.Touches(other);
	}
...
public:
	...
	bool Intersect( HandleInfo_t& handleInfo )
	{
		return IsBoxIntersectingSphere( handleInfo.m_Min, handleInfo.m_Max,m_Center, m_Radius );
	}
...
bool IsBoxIntersectingRay(Vector& min, Vector& max, const Vector& start, const Vector& delta)
{
    BBox box(min,max);
    if(box.IsPointInBox(start))
        return true;

    float dirLength = delta.Length();
	Vector dir = delta.Normalized();
    float fNear,fFar;
    int fi = ray_intersectBBox(min,max,start,dir,fNear,fFar);
...
public:
	...
	bool Intersect( HandleInfo_t& handleInfo )
	{
		...
		else
		{
			...
			Vector bmin, bmax;
            bmax = handleInfo.m_Max + m_pRay->m_Extents;
            bmin = handleInfo.m_Min - m_pRay->m_Extents;
	...
	const Ray_t& ray, bool coarseTest, IPartitionEnumerator* pIterator )
{
	...
	{
		Vector mins, maxs;
        mins = ray.m_Start - ray.m_Extents;
        maxs = ray.m_Start + ray.m_Extents;
