...
HRESULT DXUTPrepareDevice( IDirect3DDevice9* pd3dDevice );
...
HRESULT DXUTInit( bool bParseCommandLine, bool bHandleDefaultHotkeys, bool bShowMsgBoxOnError )
{
    ...
    if( pD3D == NULL )
    {
        ...
        HINSTANCE hInstD3D9 = LoadLibrary( wszPath );
        if( hInstD3D9 != NULL ) 
        {
            LPDIRECT3DCREATE9 pDirect3DCreate9 = (LPDIRECT3DCREATE9)GetProcAddress( hInstD3D9, "Direct3DCreate9" );
            if( NULL != pDirect3DCreate9 )
            {
...
void DXUTParseCommandLine()
{
    ...
    for(;;)
    {
        ...
        if( _wcsnicmp( strCmdLine, strFlag, nFlagLen ) == 0 )
        {
            float fTimePerFrame;
            if( DXUTGetCmdParam( strCmdLine, strFlag, nFlagLen ) )
                fTimePerFrame = (float)_wtof(strFlag);
            else
...
HRESULT DXUTChangeDevice( DXUTDeviceSettings* pNewDeviceSettings, IDirect3DDevice9* pd3dDeviceFromApp, bool bForceRecreate )
{
    ...
    else
    {
        ...
        if( pd3dDeviceFromApp == NULL )
        {
            ...
            UINT AdapterToUse = pNewDeviceSettings->AdapterOrdinal;
            D3DDEVTYPE  DeviceType = pNewDeviceSettings->DeviceType;
            ...
            hr = pD3D->CreateDevice( AdapterToUse, DeviceType , 
...
HRESULT DXUTInitialize3DEnvironment()
{
    ...
    hr = DXUTPrepareDevice( pd3dDevice );
    if( FAILED(hr) )
        return hr;
    ...
    if( FAILED(hr) )
    {
        if( hr == DXUTERR_MEDIANOTFOUND )
            DXUTDisplayErrorMessage( DXUTERR_MEDIANOTFOUND );
        return hr;
    }
    ...
    if( FAILED(hr) )  
    {
        if( hr == DXUTERR_MEDIANOTFOUND )
            ...
            DXUTDisplayErrorMessage( DXUTERR_CREATINGDEVICEOBJECTS );
    ...
    else
    {
        ...
        if( FAILED(hr) )
        {
            if( hr == DXUTERR_MEDIANOTFOUND )
                ...
                DXUTDisplayErrorMessage( DXUTERR_RESETTINGDEVICEOBJECTS );
...
HRESULT DXUTReset3DEnvironment()
{
    ...
    if( FAILED(hr) )  
        return DXUT_ERR( L"pd3dDevice->Reset", hr );
    ...
    DXUTGetGlobalResourceCache().OnResetDevice( pd3dDevice );
    if( FAILED(hr) )
        return hr;
...
#endif
        hr = DXUTSetDeviceCursor( pd3dDevice, hCursor, false );
        if( FAILED(hr) )
            return DXUT_ERR_MSGBOX( L"DXUTSetDeviceCursor", hr );
    ...
    switch( uMsg )
    {
       ...
       case WM_ENTERMENULOOP:
        ...
        case WM_CLOSE:
        {
            ...
            // Tim: Added for exception-free shutdowns
