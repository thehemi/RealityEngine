...
#endif
#ifndef WM_XBUTTONUP
#define WM_XBUTTONUP 0x020C // (not always defined)8
#endif
...
void DrawRectangle(int x,int y, int width, int height, D3DCOLOR color)
{
    ...
    {
        (float)x,           (float)y,            0.5f, 1.0f, color, 0.0f, 0.5f, 
        (float)x + width, (float)y,            0.5f, 1.0f, color, 1.0f, 0.5f,
        (float)x + width, (float)y + height, 0.5f, 1.0f, color, 1.0f, 1.0f, 
        (float)x,           (float)y + height, 0.5f, 1.0f, color, 0.0f, 1.0f, 
    };
...
void DrawRectangle(RECT & rect,CDXUTDialog * dialog, D3DCOLOR color)
{
    OffsetRect(&rect,dialog->GetX(),dialog->GetY());
    DrawRectangle(rect.left,rect.top,rect.right-rect.left,rect.bottom-rect.top,color);
    OffsetRect(&rect,-dialog->GetX(),-dialog->GetY());
}
...
//--------------------------------------------------------------------------------------
void CDXUTDialog::RemoveAt( int index )
{
    CDXUTControl* pControl = m_Controls.GetAt( index );
    // Clean focus first
    ClearFocus();
...
{   
    ...
    RECT fullBGRect;
    SetRect( &fullBGRect, 0, 0, 256, 256 );
    CDXUTElement bgElement;

    bgElement.SetTexture( 1, &fullBGRect );
    bgElement.TextureColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(255, 255, 255, 255);
    bgElement.FontColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(255, 255, 255, 255);
    bgElement.SetFont( 0, D3DCOLOR_ARGB(255, 255, 255, 255), DT_LEFT | DT_VCENTER );
    // Pre-blend as we don't need to transition the state
    bgElement.TextureColor.Blend( DXUT_STATE_NORMAL, 10.0f );
    bgElement.FontColor.Blend( DXUT_STATE_NORMAL, 10.0f );

    SetRect( &fullBGRect, 0, 0, m_width, m_height );

    //Render Background if set to render
    if( !m_bMinimized && m_bDisplayBG )
        DrawSprite( &bgElement, &fullBGRect );
...
bool CDXUTDialog::MsgProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
    ...
    if( m_bCaption )
    {
        if( uMsg == WM_LBUTTONDOWN || uMsg == WM_LBUTTONDBLCLK )
        {
            ...
            if( uMsg == WM_LBUTTONUP )
            {
                ...
                if( (mousePoint.x == clickPoint.x && mousePoint.y == clickPoint.y) && (mousePoint.x >= m_x && mousePoint.x < m_x + m_width &&
                    mousePoint.y >= m_y && mousePoint.y < m_y + m_nCaptionHeight) )
                {
    ...
    // TIM: Check for dragging
    if(uMsg == WM_MOUSEMOVE && bDrag && bDraggable){
        POINT mousePoint = { short(LOWORD(lParam)), short(HIWORD(lParam)) };

        // Update drag location
        SetLocation(windowStart.x + (mousePoint.x - clickPoint.x), windowStart.y + (mousePoint.y - clickPoint.y));
    ...
    if(bDrag) // TIM: So no more windows process while draggign
        return true;
...
CDXUTControl* CDXUTDialog::GetControlAtPoint( POINT pt )
{
    ...
    for( int i=0; i < m_Controls.GetSize(); i++ )
    {
        ...
        if( pControl->GetVisible() && pControl->ContainsPoint( pt ) )
        {
            return pControl;
        }
    }
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddTabControl( int ID,  LPCWSTR strText, int x, int y, 
                                   int width, int height, CDXUTab** ppCreated)
{
    HRESULT hr = S_OK;

    CDXUTab* pTab = new CDXUTab( this );
    ...
    // inside call initcontrol which links up element based on initdefault elements
    // cause the constructor sets type
    hr = AddControl( pTab );
    if( FAILED(hr) )
        return hr;

    // Set the ID and general data
    pTab->SetID( ID ); 
    pTab->SetText( strText );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddTabPagesControl( int ID, int x, int y, int width, int height, CDXUTTabPages** ppCreated )
{
    HRESULT hr = S_OK;

    CDXUTTabPages* pTabPages = new CDXUTTabPages( this );
    ...
    hr = AddControl( pTabPages );
    if( FAILED(hr) )
        return hr;

    // Set the ID and list index
    pTabPages->SetID( ID ); 
    pTabPages->SetLocation( x, y );
    pTabPages->SetSize( width, height );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddSlider( int ID, int x, int y, int width, int height, int min, int max, int value, bool bIsDefault, CDXUTSlider** ppCreated, float UnitScale)
{
    HRESULT hr = S_OK;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddPropertiesList( int ID, int x, int y, int width, int height, DWORD dwStyle, CDXUTPropertiesList** ppCreated )
{
    HRESULT hr = S_OK;
    CDXUTPropertiesList *pListBox = new CDXUTPropertiesList( this );
...
HRESULT CDXUTDialog::AddControl( CDXUTControl* pControl )
{
	...
	// if it's a tabpages set, add all the controls in the tabpages pages
	if(pControl->GetType() == DXUT_CONTROL_TABPAGES)
	{
		for(int i = 0; i < ((CDXUTTabPages*)pControl)->m_Pages.GetSize();i++)
		{
			DXUTPage* page = &((CDXUTTabPages*)pControl)->m_Pages[i];
			InitControl(page->m_Tab);
			m_Controls.Add(page->m_Tab);
			for(int n = 0; n < page->m_Controls.GetSize();n++)
			{
				InitControl(page->m_Controls[n]);
				m_Controls.Add(page->m_Controls[n]);
			}
		}
	}
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::InsertControl( int index, CDXUTControl* pControl )
{
    HRESULT hr = S_OK;
    ...
    // Add to the list
    hr = m_Controls.Insert(index, pControl );
    if( FAILED(hr) )
    {
        return DXTRACE_ERR( L"CGrowableArray::Add", hr );
    }
...
//--------------------------------------------------------------------------------------
bool CDXUTDialog::Contains( CDXUTControl* pControl )
{
    return m_Controls.Contains(pControl);
}
...
HRESULT CDXUTDialogResourceManager::CreateFont( UINT iFont )
{
    ...
    V_RETURN( D3DXCreateFont( m_pd3dDevice, pFontNode->nHeight, 0, pFontNode->nWeight*1.2, 1, FALSE, DEFAULT_CHARSET, 
...
HRESULT CDXUTDialogResourceManager::CreateTexture( UINT iTexture )
{
    ...
    // TIM: Search with our code, not DX's paths
#ifndef _BUILDTOOL
    string temp = ToAnsi(pTextureNode->strFilename);
    FindMedia(temp,"Textures");
    wstring strPath = ToUnicode(temp);
    ...
    // Create texture
    D3DXIMAGE_INFO info;
    hr =  D3DXCreateTextureFromFileEx( m_pd3dDevice, strPath.c_str(), D3DX_DEFAULT, D3DX_DEFAULT, 
...
{
    SetTexture( 0, L"UI\\DXUTControls.tga" );
    SetTexture( 1, L"UI\\wndBg.tga" );
    ...
    //-------------------------------------
    Element.SetFont( 0 );
    Element.FontColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB( 255, 250, 250, 250 );
    ...
    //-------------------------------------
    // CDXUTTab - Tap page tab
    //-------------------------------------

    SetRect( &rcTexture, 332, 6, 391, 40 );
    Element.SetTexture( 0, &rcTexture );
    Element.SetFont( 0 );
    ...
    Element.FontColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB( 255, 250, 250, 250 );
    Element.FontColor.States[ DXUT_STATE_PRESSED ] = D3DCOLOR_ARGB( 255, 250, 250, 250 );
    Element.SetFont( 0, D3DCOLOR_ARGB( 255, 255, 255, 255 ), DT_CENTER | DT_VCENTER );

    // Assign the Element
    SetDefaultElement( DXUT_CONTROL_TAB, 0, &Element );

    //-------------------------------------
    // CDXUTTab - Tap page tab inactive layer
    //-------------------------------------
    SetRect( &rcTexture, 422, 8, 482, 42 );
    Element.SetTexture( 0, &rcTexture, D3DCOLOR_ARGB(0, 255, 255, 255) );
    ...
    Element.FontColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB( 255, 250, 250, 250 );
    Element.FontColor.States[ DXUT_STATE_PRESSED ] = D3DCOLOR_ARGB( 255, 250, 250, 250 );
    Element.TextureColor.States[ DXUT_STATE_FOCUS ] = D3DCOLOR_ARGB(255, 2550, 255, 255);
    Element.SetFont( 0, D3DCOLOR_ARGB( 255, 150, 150, 150 ), DT_CENTER | DT_VCENTER );

    // Assign the Element
    SetDefaultElement( DXUT_CONTROL_TAB, 1, &Element );

    //-------------------------------------
    // CDXUTTabPages - Tap page texture
    //-------------------------------------		

    SetRect( &rcTexture, 299, 48, 505, 196 );
    Element.SetTexture( 0, &rcTexture);   
    Element.TextureColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(255, 255, 255, 255);
    ...
    // Assign the Element
    SetDefaultElement( DXUT_CONTROL_TABPAGES, 0, &Element );
    ...
    // Assign the Element
    SetDefaultElement( DXUT_CONTROL_TABPAGES, 1, &Element );
    ...
    Element.SetTexture( 0, &rcTexture );
    Element.SetFont( 0, D3DCOLOR_ARGB( 255, 204, 204, 204 ), DT_LEFT | DT_VCENTER );
    Element.FontColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB( 255, 250, 250, 250 );
    Element.FontColor.States[ DXUT_STATE_DISABLED ] = D3DCOLOR_ARGB( 200, 200, 200, 200 );
    Element.TextureColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(150, 255, 255, 255);
    Element.TextureColor.States[ DXUT_STATE_FOCUS ] = D3DCOLOR_ARGB(200, 255, 255, 255);
    ...
    Element.SetTexture( 0, &rcTexture );
    Element.FontColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB( 255, 250, 250, 250 );
    Element.FontColor.States[ DXUT_STATE_FOCUS ] = D3DCOLOR_ARGB( 255, 250, 250, 250 );
    Element.TextureColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(150, 255, 255, 255);
    Element.TextureColor.States[ DXUT_STATE_FOCUS ] = D3DCOLOR_ARGB(200, 255, 255, 255);
    Element.TextureColor.States[ DXUT_STATE_DISABLED ] = D3DCOLOR_ARGB(70, 255, 255, 255);
    ...
    // Assign the Element
    SetDefaultElement( DXUT_CONTROL_PROPERTIESLIST, 0, &Element );
    ...
    Element.SetFont( 0, D3DCOLOR_ARGB(255, 250, 250, 250), DT_LEFT | DT_TOP );
    ...
    // Assign the Element
    SetDefaultElement( DXUT_CONTROL_PROPERTIESLIST, 1, &Element );
}
...
{
    if( bChecked && bClearGroup && m_pDialog)
        m_pDialog->ClearRadioButtonGroup( m_nButtonGroup );
...
HRESULT CDXUTComboBox::AddItem( const WCHAR* strText, void* pData )
{
    ...
    // Check to see if this item already exists, if so just change the data
...
//--------------------------------------------------------------------------------------
bool CDXUTComboBox::ContainsItem( const WCHAR* strText )
{
    return ( -1 != FindItem( strText ) );
}
...
//--------------------------------------------------------------------------------------
int CDXUTComboBox::FindItem( const WCHAR* strText )
{
    if( strText == NULL )
        return -1;
...
void CDXUTSlider::Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
    ...
    pElement = m_Elements.GetAt( 0 );
    pElement->FontColor.Blend( iState, fElapsedTime );
    m_pDialog->DrawText( ToUnicode(ToStr(GetValue()*m_UnitScale)).c_str(), pElement, &m_rcBoundingBox, true );
}
...
HRESULT CDXUTListBox::AddItem( const WCHAR *wszText, void *pData )
{
    ...
    lstrcpyW( pNewItem->strText, wszText );
...
//--------------------------------------------------------------------------------------
CDXUTab::CDXUTab( CDXUTDialog *pDialog)
{
...
//--------------------------------------------------------------------------------------
bool CDXUTab::HandleKeyboard( UINT uMsg, WPARAM wParam, LPARAM lParam )
{
    if( !m_bEnabled || !m_bVisible )
        return false;
    ...
    case WM_KEYUP:
        {
            ...
            case VK_SPACE:
                ...
                {
                    m_bPressed = false;
                    m_pDialog->SendEvent( EVENT_TAB_CLICKED, true, this );
                    parentPages->TabPressed(this);
                }
                return true;
            }
        }
...
//--------------------------------------------------------------------------------------
bool CDXUTab::HandleMouse( UINT uMsg, POINT pt, WPARAM wParam, LPARAM lParam )
{
    if( !m_bEnabled || !m_bVisible )
        return false;
...
//--------------------------------------------------------------------------------------
void CDXUTab::Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
    DXUT_CONTROL_STATE iState = DXUT_STATE_NORMAL;
    ...
    if(this->parentPages->isSelected(this))iState = DXUT_STATE_FOCUS;
    else iState = DXUT_STATE_HIDDEN;
    ...
    if(this->parentPages->isSelected(this))iState = DXUT_STATE_HIDDEN;
    else iState = DXUT_STATE_FOCUS;
...
//--------------------------------------------------------------------------------------
CDXUTTabPages::CDXUTTabPages( CDXUTDialog *pDialog ) 
{
    m_Type = DXUT_CONTROL_TABPAGES;
    m_pDialog = pDialog;	
    Displacement=10;
}
...
//--------------------------------------------------------------------------------------
void CDXUTTabPages::AddTabPage(const WCHAR *wszText, int width)
{
    DXUTPage page;
    lstrcpyW(page.strText , wszText );
    page.bSelected = false;

    static int IDFORTABS=2134;

    //add in tab button when you add page
    CDXUTab *newTab;
	if(m_pDialog)
	{
    m_pDialog->AddTabControl(IDFORTABS, wszText, 
        (Displacement) + this->m_x, this->m_y, width, 30, &newTab);
	}
	else
	{
    newTab = new CDXUTab( NULL );
    newTab->SetID( IDFORTABS ); 
    newTab->SetText( wszText );
    newTab->SetLocation(  (Displacement) + this->m_x, this->m_y );
    newTab->SetSize( width, 30 );   
	}

    newTab->parentPages=this;
...
//--------------------------------------------------------------------------------------
void CDXUTTabPages::Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
    /*
    if( m_bVisible == false )
    return;
    ...
    RECT pageRect;
    pageRect.left=30; //BORDER
    pageRect.top=30; //BORDER
    pageRect.bottom= 30 + pFontNode->nHeight;
    pageRect.right = 30 + 72; //border + width

    OffsetRect(&pageRect,this->m_x ,this->m_y);

    for (int i=0;i<m_Pages.GetSize();i++)
    {
    DrawRectangle(pageRect, this->m_pDialog, D3DCOLOR_ARGB(230, 10, 13,80) );
    //  m_pDialog->DrawText(m_Pages[i].strText,pElement,&pageRect,false);
    pageRect.right += 72;
    }*/
...
//--------------------------------------------------------------------------------------
// CDXUTPropertiesList class
//--------------------------------------------------------------------------------------

//--------------------------------------------------------------------------------------
CDXUTPropertiesList::CDXUTPropertiesList( CDXUTDialog *pDialog ) :
m_ScrollBar( pDialog )
{
    m_Type = DXUT_CONTROL_PROPERTIESLIST;
    m_pDialog = pDialog;
...
//--------------------------------------------------------------------------------------
CDXUTPropertiesList::~CDXUTPropertiesList()
{
    RemoveAllItems();
}
...
//--------------------------------------------------------------------------------------
void CDXUTPropertiesList::UpdateRects()
{
    CDXUTControl::UpdateRects();

    DXUTFontNode* pFontNode = DXUTGetGlobalDialogResourceManager()->GetFontNode( m_Elements.GetAt( 0 )->iFont );

    m_rcText= m_rcSelection = m_rcBoundingBox;

    m_rcText.right = m_labelWidth+m_rcText.left;

    m_rcSelection.right-=m_nSBWidth;
    InflateRect( &m_rcSelection, -m_nBorder, 0);
    OffsetRect(&m_rcText,0,m_nBorder);
    InflateRect( &m_rcText, -m_nBorder, 0);
    InflateRect( &m_rcText, -m_nMargin, 0 );
    ...
    if( pFontNode && pFontNode->nHeight )
    {
        m_ScrollBar.SetPageSize( (RectHeight( m_rcText )-m_nBorder*2) / (pFontNode->nHeight +5));
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTPropertiesList::AddItem( const WCHAR *wszText, void *pData )
{
    DXUTPropListItem *pNewItem = new DXUTPropListItem;
    if( !pNewItem )
        return E_OUTOFMEMORY;
    ...
    SetRect( &pNewItem->rcActive, 0, 0, 0, 0 );
    pNewItem->bSelected = false;
    pNewItem->bIsTitle=false;
    pNewItem->editControl=new CDXUTEditBox(this->m_pDialog);
    this->m_pDialog->InitControl(pNewItem->editControl);
    ((CDXUTEditBox*)pNewItem->editControl)->SetBorderWidth(0);
    pNewItem->editControl->m_bIsDefault=false;

    //this->m_pDialog->AddEditBox(m_cntID,NULL,0,0,30,30,false,(CDXUTEditBox**) &pNewItem->editControl);
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTPropertiesList::AddButton( const WCHAR *wszText,int ID, void *pData ,LPCWSTR text)
{
    DXUTPropListItem *pNewItem = new DXUTPropListItem;
    if( !pNewItem )
        return E_OUTOFMEMORY;
    ...
    pNewItem->editControl=new CDXUTButton(this->m_pDialog);
    pNewItem->editControl->SetID(ID);
    this->m_pDialog->InitControl(pNewItem->editControl);
    ((CDXUTButton*)pNewItem->editControl)->SetText(text);
    pNewItem->editControl->m_bIsDefault=false;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTPropertiesList::AddItem( const WCHAR *wszText,int ID, void *pData ,LPCWSTR text)
{
    DXUTPropListItem *pNewItem = new DXUTPropListItem;
    if( !pNewItem )
        return E_OUTOFMEMORY;
    ...
    this->m_pDialog->InitControl(pNewItem->editControl);
    ((CDXUTEditBox*)pNewItem->editControl)->SetBorderWidth(0);
    ((CDXUTEditBox*)pNewItem->editControl)->SetText(text,false);
    pNewItem->editControl->m_bIsDefault=false;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTPropertiesList::AddComboBox( const WCHAR *wszText,int ID, void *pData )
{
    DXUTPropListItem *pNewItem = new DXUTPropListItem;
    if( !pNewItem )
        return E_OUTOFMEMORY;
    ...
    pNewItem->bSelected = false;
    pNewItem->bIsTitle=false;
    pNewItem->editControl=new CDXUTComboBox(this->m_pDialog);
    pNewItem->editControl->SetID(ID);
    this->m_pDialog->InitControl(pNewItem->editControl);
    pNewItem->editControl->m_bIsDefault=false;
    HRESULT hr = m_Items.Add( pNewItem );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTPropertiesList::AddSlider( const WCHAR *wszText,int ID,int rangeMin,int rangeMax,int intialValue, void *pData )
{
    DXUTPropListItem *pNewItem = new DXUTPropListItem;
    if( !pNewItem )
        return E_OUTOFMEMORY;
    ...
    pNewItem->bSelected = false;
    pNewItem->bIsTitle=false;
    pNewItem->editControl=new CDXUTSlider(this->m_pDialog);
    this->m_pDialog->InitControl(pNewItem->editControl);
    pNewItem->editControl->SetID(ID);
    ((CDXUTSlider*)pNewItem->editControl)->SetRange(rangeMin,rangeMax);
    ((CDXUTSlider*)pNewItem->editControl)->SetValue(intialValue);
    pNewItem->editControl->m_bIsDefault=false;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTPropertiesList::AddCheckBox( const WCHAR *wszText,int ID,bool checked, void *pData )
{
    DXUTPropListItem *pNewItem = new DXUTPropListItem;
    if( !pNewItem )
        return E_OUTOFMEMORY;
    ...
    pNewItem->bSelected = false;
    pNewItem->bIsTitle=false;
    pNewItem->editControl=new CDXUTCheckBox(this->m_pDialog);
    this->m_pDialog->InitControl(pNewItem->editControl);
    pNewItem->editControl->SetID(ID);
    ((CDXUTCheckBox*)pNewItem->editControl)->SetChecked(checked);
    pNewItem->editControl->m_bIsDefault=false;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTPropertiesList::AddStatic( const WCHAR *wszText,int ID,LPCWSTR text, void *pData )
{
    DXUTPropListItem *pNewItem = new DXUTPropListItem;
    if( !pNewItem )
        return E_OUTOFMEMORY;
    ...
    pNewItem->bSelected = false;
    pNewItem->bIsTitle=false;
    pNewItem->editControl=new CDXUTStatic(this->m_pDialog);
    this->m_pDialog->InitControl(pNewItem->editControl);
    pNewItem->editControl->SetID(ID);
    ((CDXUTStatic*)pNewItem->editControl)->SetText(text);
    ((CDXUTStatic*)pNewItem->editControl)->m_Elements.GetAt(0)->dwTextFormat=DT_LEFT | DT_VCENTER;
    ((CDXUTStatic*)pNewItem->editControl)->m_Elements.GetAt(0)->FontColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(255, 200, 200, 200);
    ((CDXUTStatic*)pNewItem->editControl)->m_Elements.GetAt(0)->FontColor.Current = D3DCOLOR_ARGB(255, 0, 0, 0);
    pNewItem->editControl->m_bIsDefault=false;
    HRESULT hr = m_Items.Add( pNewItem );
    if( SUCCEEDED( hr ) )
        m_ScrollBar.SetTrackRange( 0, m_Items.GetSize() );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTPropertiesList::AddTitle( const WCHAR *wszText,int ID,LPCWSTR text, void *pData )
{
    DXUTPropListItem *pNewItem = new DXUTPropListItem;
    if( !pNewItem )
        return E_OUTOFMEMORY;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTPropertiesList::InsertItem( int nIndex, const WCHAR *wszText, void *pData )
{
    DXUTPropListItem *pNewItem = new DXUTPropListItem;
    if( !pNewItem )
        return E_OUTOFMEMORY;
...
//--------------------------------------------------------------------------------------
void CDXUTPropertiesList::RemoveItem( int nIndex )
{
    if( nIndex < 0 || nIndex >= (int)m_Items.GetSize() )
        return;

    DXUTPropListItem *pItem = m_Items.GetAt( nIndex );

    //	this->m_pDialog->RemoveControl(pItem->editControl->GetID());
    delete pItem->editControl;
    delete pItem;
    m_Items.Remove( nIndex );
    m_ScrollBar.SetTrackRange( 0, m_Items.GetSize() );
    if( m_nSelected >= (int)m_Items.GetSize() )
...
//--------------------------------------------------------------------------------------
void CDXUTPropertiesList::RemoveItemByText( WCHAR *wszText )
{
}


//--------------------------------------------------------------------------------------
void CDXUTPropertiesList::RemoveItemByData( void *pData )
{
}


//--------------------------------------------------------------------------------------
void CDXUTPropertiesList::RemoveAllItems()
{
    for( int i = 0; i < m_Items.GetSize(); ++i )
    {
        DXUTPropListItem *pItem = m_Items.GetAt( i );
        delete pItem;
    }
...
//--------------------------------------------------------------------------------------
DXUTPropListItem *CDXUTPropertiesList::GetItem( int nIndex )
{
    if( nIndex < 0 || nIndex >= (int)m_Items.GetSize() )
        return NULL;
...
// nPreviousSelected.
// Returns -1 on error or if no item is selected.
int CDXUTPropertiesList::GetSelectedIndex( int nPreviousSelected )
{
    if( nPreviousSelected < -1 )
        return -1;
...
//--------------------------------------------------------------------------------------
void CDXUTPropertiesList::SelectItem( int nNewIndex )
{
    // If no item exists, do nothing.
    if( m_Items.GetSize() == 0 )
        return;
    ...
    if( nOldSelected != m_nSelected )
    {
        if (((DXUTPropListItem*)m_Items.GetAt( m_nSelected ))->bIsTitle) return;

        // Update selection start
        m_nSelStart = m_nSelected;
        m_pDialog->RequestFocus(((DXUTPropListItem*)m_Items.GetAt( m_nSelected ))-> editControl);
...
CDXUTControl * CDXUTPropertiesList::GetControlByID(int ID)
{
    for( int i =0; i < (int)m_Items.GetSize(); i++ )
    {
        DXUTPropListItem *pItem = m_Items.GetAt( i );
        if (pItem->editControl->GetID()==ID)
            return pItem->editControl;
    }
    return NULL;
}

//--------------------------------------------------------------------------------------
bool CDXUTPropertiesList::HandleKeyboard( UINT uMsg, WPARAM wParam, LPARAM lParam )
{
    if( !m_bEnabled || !m_bVisible )
        return false;
    ...
    {
        DXUTPropListItem *pItem = m_Items.GetAt( i );
        if (pItem->editControl->HandleKeyboard(uMsg,wParam,lParam))
            return true;
    }
        ...
        case VK_END:
            ...
            if( nOldSelected != m_nSelected )
            {
                if( m_dwStyle & MULTISELECTION )
                {
                    ...
                    for( int i = 0; i < (int)m_Items.GetSize(); ++i )
                    {
                        DXUTPropListItem *pItem = m_Items[i];
                        pItem->bSelected = false;
                    }
...
//--------------------------------------------------------------------------------------
bool CDXUTPropertiesList::HandleMouse( UINT uMsg, POINT pt, WPARAM wParam, LPARAM lParam )
{
    if( !m_bEnabled || !m_bVisible )
        return false;
    ...
    {
        DXUTPropListItem *pItem = m_Items.GetAt( i );
        if (pItem->editControl->HandleMouse(uMsg,pt,wParam,lParam))
        {
            if (!pItem->bIsTitle)
            {
                for( int j = 0; j < (int)m_Items.GetSize(); ++j )
                {
                    DXUTPropListItem *pItem1 = m_Items.GetAt( j );
                    pItem1->bSelected = false;
                }
                SelectItem(i);
            }
            return true;
        ...
        if( m_Items.GetSize() > 0 && PtInRect( &m_rcSelection, pt ) )
        {
                ...
                nClicked < m_ScrollBar.GetTrackPos() + m_ScrollBar.GetPageSize() )
            {
                ...
                if( m_dwStyle & MULTISELECTION )
                {
                    ...
                    DXUTPropListItem *pSelItem = m_Items.GetAt( m_nSelected );
                    if( ( wParam & (MK_SHIFT|MK_CONTROL) ) == MK_CONTROL )
                    {
                        // Control click. Reverse the selection of this item.
                        ...
                        if( ( wParam & (MK_SHIFT|MK_CONTROL) ) == MK_SHIFT )
                        {
                            ...
                            {
                                DXUTPropListItem *pItem = m_Items.GetAt( i );
                                pItem->bSelected = !pSelItem->bIsTitle;
                            }
                        } else
                            if( ( wParam & (MK_SHIFT|MK_CONTROL) ) == ( MK_SHIFT|MK_CONTROL ) )
                            {
                                ...
                                pSelItem->bSelected = !pSelItem->bIsTitle;
                            ...
                            } else
                            {
                                ...
                                for( int i = 0; i < (int)m_Items.GetSize(); ++i )
                                {
                                    DXUTPropListItem *pItem = m_Items.GetAt( i );
                                    pItem->bSelected = false;
                                    pSelItem->editControl->m_bHasFocus=false;
                                }
                                m_pDialog->RequestFocus(pSelItem->editControl);
                                pSelItem->bSelected = !pSelItem->bIsTitle;
                            }
                }  // End of multi-selection case
    ...
    case WM_LBUTTONUP:
        {
            ...
            if( m_nSelected != -1 )
            {
                ...
                if ( !m_Items[m_nSelected]->bIsTitle)
                    m_Items[m_nSelected]->bSelected = m_Items[m_nSelStart]->bSelected;
                m_pDialog->RequestFocus(m_Items[m_nSelected]->editControl);
...
//--------------------------------------------------------------------------------------
void CDXUTPropertiesList::Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
    if( m_bVisible == false )
        return;
    ...
    CDXUTElement* pSelElement = m_Elements.GetAt( 1 );
    pSelElement->dwTextFormat=DT_VCENTER;
    pSelElement->TextureColor.Blend( DXUT_STATE_NORMAL, fElapsedTime );
    pSelElement->FontColor.Blend( DXUT_STATE_NORMAL, fElapsedTime );
    ...
    POINT vLine[2];
    vLine[0].x=m_rcText.right+4;

    vLine[0].y=m_rcBoundingBox.top;

    vLine[1].x=m_rcText.right+4;
    vLine[1].y=m_rcBoundingBox.bottom;
    if (m_pDialog->GetCaptionEnabled())
    {
        int cHeight=m_pDialog->GetCaptionHeight();
        vLine[0].y+=cHeight+m_nBorder;
        vLine[1].y+=cHeight-m_nBorder;
    }
    m_pDialog->DrawPolyLine(vLine,2,D3DCOLOR_ARGB(255, 200, 200, 200));
    ...
    if( m_Items.GetSize() > 0 )
    {
        ...
        RECT rc = m_rcText;
        RECT rcSel = m_rcSelection;
        rc.bottom = rc.top + DXUTGetGlobalDialogResourceManager()->GetFontNode( pElement->iFont )->nHeight + m_nBorder;
        ...
        if( !bSBInit )
        {
            // Update the page size of the scroll bar
            if( m_nTextHeight )
                m_ScrollBar.SetPageSize( (RectHeight( m_rcText )-m_nBorder*2 )/ m_nTextHeight );
            else
                m_ScrollBar.SetPageSize( RectHeight( m_rcText )-m_nBorder*2 );
            bSBInit = true;
        }
        ...
        for( int i = m_ScrollBar.GetTrackPos(); i < (int)m_Items.GetSize(); ++i )
        {
            if( rc.bottom > m_rcText.bottom - m_nBorder )
                break;
            ...
            if( !( m_dwStyle & MULTISELECTION ) && i == m_nSelected )
                ...
                {
                    rcSel.top = rc.top; rcSel.bottom = rc.bottom+3;
                    if (i)
                    {
                        POINT line[2];
                        line[0].x=rcSel.left;
                        line[0].y=rcSel.top;
                        line[1].x=rcSel.right;
                        line[1].y=rcSel.top;
                        if (m_pDialog->GetCaptionEnabled())
                        {
                            int cHeight=m_pDialog->GetCaptionHeight();
                        ...
                        m_pDialog->DrawPolyLine(line,2,D3DCOLOR_ARGB(255, 200, 200, 200));
                    }
                    if (pItem->bIsTitle)
                    {
                        InflateRect(&rcSel,0,-2);
                        OffsetRect( &rcSel, 0, -2 );
                        m_pDialog->DrawRect(&rcSel,D3DCOLOR_ARGB(255, 200, 200, 200));
                    }
                    //OffsetRect( &rc, 0, 2 );
                    m_pDialog->DrawText( pItem->strText, pElement, &rc );
                }
                //OffsetRect( &rc, 0, -2 );
                pItem->editControl->SetLocation(m_labelWidth,rc.top+1);
                pItem->editControl->SetSize((m_rcBoundingBox.right -m_rcBoundingBox.left) - m_nSBWidth -m_labelWidth - m_nBorder,m_nTextHeight-2);
                pItem->editControl->Render(pd3dDevice,fElapsedTime);
                OffsetRect( &rc, 0, m_nTextHeight );
        }
    }
    ...
    m_ScrollBar.Render( pd3dDevice, fElapsedTime );
    //m_pDialog->DrawRect(&m_rcBoundingBox,D3DCOLOR_ARGB(255, 200, 200, 200));
}
...
{
    if (m_rcText.right <= m_rcText.left) return;
    ...
    if( nX < nX1st )
    {
        while( nX < nXOffset )
        {
            int nScrollAmount = ( m_rcText.right - m_rcText.left ) / EDITBOX_SCROLLEXTENT;
            if( nScrollAmount == 0 ) nScrollAmount = 20;
            nXOffset -= nScrollAmount;
            int n1stChar, nTrail;
            m_Buffer.XtoCP( nXOffset, &n1stChar, &nTrail );
            m_Buffer.CPtoX( n1stChar, FALSE, &nXOffset );
            m_nFirstVisible = n1stChar;
        }
    }
    else
        ...
        {
            while( nX2 > nXOffset + m_rcText.right - m_rcText.left )
            {
                int nScrollAmount = ( m_rcText.right - m_rcText.left ) / EDITBOX_SCROLLEXTENT;
                if( nScrollAmount == 0 ) nScrollAmount = 20;
                nXOffset += nScrollAmount;
                int n1stChar, nTrail;
                m_Buffer.XtoCP( nXOffset, &n1stChar, &nTrail );
                m_Buffer.CPtoX( n1stChar, nTrail, &nXOffset );
                if( nTrail && n1stChar < m_Buffer.GetTextSize() )
                    m_nFirstVisible = n1stChar + 1;
                else
                    m_nFirstVisible = n1stChar;
            }
        }
    ...
    if( OpenClipboard( NULL ) )
    {
        ...
        if( handle )
        {
            ...
            if( pwszText )
            {
                ...
                while( *pwszText != L'\0' )
                {
                    if( m_Buffer.InsertChar( m_nCaret, *pwszText ) )
...
void CDXUTEditBox::Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
    ...
    // Temporary. Need a way to let control initialize certain data
    // after we have the window.  For now, set the DC every time we render.
    if( !m_Buffer.GetDC() )
    {
        ...
        if( pElement )
        {
            ...
            m_Buffer.SetDC( pFontNode->pFont->GetDC() );
            PlaceCaret( m_nCaret );  // Call PlaceCaret again now that we have the DC,
...
void CDXUTIMEEditBox::GetPrivateReadingString()
{
    ...
    if( s_bHorizontalReading )
    {
        ...
        for( UINT i = 0; i < s_CandList.dwCount; ++i )
        {
            ...
            lstrcat( s_wszReadingString, s_CandList.awszCandidate[i] );
        }
    }
...
void CDXUTIMEEditBox::RenderComposition( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
    ...
    pElement->FontColor.Current = m_CompColor;
    m_pDialog->DrawText( s_CompString.GetBuffer(), pElement, &rc );

    // For target-converted characters, we display a different
    // background color so they appear highlighted.
        ...
        *pcComp != L'\0'; ++pcComp, ++pAttr )
    {
        if( *pAttr == ATTR_TARGET_CONVERTED )
        {
            ...
            RECT rcTarget = { rc.left + nXLeft, rc.top, rc.left + nXRight, rc.bottom };
            m_pDialog->DrawRect( &rcTarget, m_CompTargetBkColor );
    ...
    if( m_bCaretOn )
    {
        ...
        rc.right = rc.left + nX + 1;
        rc.left += nX - 1;
        m_pDialog->DrawRect( &rc, m_CompCaretColor );
    }
}
...
void CDXUTIMEEditBox::Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
    ...
    {
        DXUTFontNode* pFontNode = m_pDialog->GetFont( pElement->iFont );
        s_CompString.SetDC( pFontNode->pFont->GetDC() );
        s_CandList.HoriCand.SetDC( pFontNode->pFont->GetDC() );
    }
...
//--------------------------------------------------------------------------------------
bool CDXUTEditBox::CUniBuffer::Grow()
{
    int nAllocateSize = m_nBufferSize ? m_nBufferSize * 2 : 256;
...
HRESULT CDXUTEditBox::CUniBuffer::Analyse()
{
    ...
    HRESULT hr = _ScriptStringAnalyse( m_hDC,
        m_pwszBuffer,
...
{
    assert( nIndex >= 0 && nIndex <= m_nTextSize );
