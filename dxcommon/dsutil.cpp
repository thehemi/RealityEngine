...
// Tim's extensions: 
// * lVolume control for primary
// * Allocation of new buffer if current one isn't free
//		* MAX_BUFFERS limit, so buffer can grow
//		* GetFreeBuffer() re-written to allocate new buffers
//		* GetNumBuffers()
//		* CWaveFile() reference counting system 
//		* CSound::Duplicate() etc
//
// FIXME: Buffers share memory, so filling one fills them all. We want to avoid
// restoring (filling) a buffer at runtime as it may get in the way of another
// Therefore, make sure AudioDevice restores all sounds at same time
//		
//-----------------------------------------------------------------------------
#define STRICT
                               ...
                               DWORD dwNumBuffers )
{
    ...
    apDSBuffer = new LPDIRECTSOUNDBUFFER[MAX_BUFFERS];//dwNumBuffers];
    if( apDSBuffer == NULL )
    {
        hr = E_OUTOFMEMORY;
        goto LFail;
...
// Duplicates a sound & buffer (fast)
// WARNING: Buffers are SHARED memory, so filling one fills the others
void CSound::Duplicate(CSound*& clone){
	clone = new CSound();
	*clone = *this; // Copy default data
	clone->m_dwNumBuffers = 1; // Don't propogate extra buffers (wasteful)
	clone->m_pWaveFile->AddRef(); // Don't let wavefile die until all sounds are deleted

	// Duplicate sound buffer
	clone->m_apDSBuffer = new LPDIRECTSOUNDBUFFER[MAX_BUFFERS];
	HRESULT hr;
	if( FAILED( hr = m_pDS->DuplicateSoundBuffer( m_apDSBuffer[0], &clone->m_apDSBuffer[0] ) ) )
	{
		MessageBox(0,"DuplicateSoundBuffer() failed (CTRL_FX being used?)",0,0);
	}
	globalBuffers++;
}
                ...
                DWORD dwNumBuffers, CWaveFile* pWaveFile, DWORD dwCreationFlags )
{
    ...
    m_apDSBuffer = new LPDIRECTSOUNDBUFFER[MAX_BUFFERS];
...
LPDIRECTSOUNDBUFFER CSound::GetFreeBuffer()
{
    ...
    if( i != m_dwNumBuffers )
		...
		// If we've created too many buffers...
		// DEBUG: Assert, as it might be a resource problem
		// RELEASE: Gracefully reuse an in-use buffer
		//assert(m_dwNumBuffers < MAX_BUFFERS);
		if(m_dwNumBuffers >= MAX_BUFFERS)
			return m_apDSBuffer[ rand() % m_dwNumBuffers ];

		// No free buffer, so allocate a new one
		HRESULT hr;
		if( FAILED( hr = m_pDS->DuplicateSoundBuffer( m_apDSBuffer[0], &m_apDSBuffer[m_dwNumBuffers] ) ) )
		{
			MessageBox(0,"DuplicateSoundBuffer() failed (CTRL_FX being used?)",0,0);
		}
		...
		return m_apDSBuffer[m_dwNumBuffers-1];
	}
}
