...
HRESULT CDXUTResourceCache::CreateFont( LPDIRECT3DDEVICE9 pDevice, UINT Height, UINT Width, UINT Weight, UINT MipLevels, BOOL Italic, DWORD CharSet, DWORD OutputPrecision, DWORD Quality, DWORD PitchAndFamily, LPCTSTR pFacename, LPD3DXFONT *ppFont )
{
    ...
    lstrcpyW( Desc.FaceName, pFacename );
...
//--------------------------------------------------------------------------------------
D3DXQUATERNION CD3DArcBall::QuatFromBallPoints(const D3DXVECTOR3 &vFrom1, const D3DXVECTOR3 &vTo1)
{
...
//--------------------------------------------------------------------------------------
// Finds and opens the readme for this sample
//--------------------------------------------------------------------------------------
VOID DXUTLaunchReadme( HWND hWnd, LPCWSTR strLoc )
{
    HRESULT hr;
    const int NUM_FILENAMES = 2;
    LPCWSTR strFilenames[] = 
    {
        L"readme.htm",
        L"readme.txt"
    };

    WCHAR strReadmePath[MAX_PATH];

    for( int i=0; i < NUM_FILENAMES; i++ )
    {
        hr = DXUTFindDXSDKMediaFileCch( strReadmePath, MAX_PATH, strFilenames[i] );
        if( SUCCEEDED(hr) )
        {
            ...
            HINSTANCE hInstShell32 = LoadLibrary(L"shell32.dll");
            if (hInstShell32 != NULL)
            {
                pShellExecute = (LPShellExecute)GetProcAddress(hInstShell32, "ShellExecuteW");
                if( pShellExecute != NULL )
                    pShellExecute( hWnd, L"open", strReadmePath, NULL, NULL, SW_SHOW );
    ...
    MessageBox( hWnd, L"Could not find readme", 
                L"DirectX SDK Sample", MB_ICONWARNING | MB_OK );
}
...
HRESULT CDXUTDirectionWidget::OnRender( D3DXCOLOR color, D3DXMATRIX* pmView, 
                                        D3DXMATRIX* pmProj, D3DXVECTOR3* pEyePt )
{
    m_mView = *pmView;
